<script lang="ts">
	...
	let uidToDelete: string = $state('');

	let totalCount = $derived(data.totalCount || 0);
	let startIndex = $derived((data.currentPage - 1) * data.limit + 1);
	let endIndex = $derived(Math.min(data.currentPage * data.limit, totalCount));
