	...
	<div class="navbar-end">
		...
		<div class="hidden items-center space-x-2 lg:flex">
			...
			<div class="dropdown dropdown-end">
				<button
					class="btn btn-ghost transition-colors hover:bg-primary/10 {dropOpen
						? 'bg-primary/5'
						: ''}"
					on:click|stopPropagation={() => (dropOpen = !dropOpen)}
				>
					<svg class="mr-1 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path
							stroke-linecap="round"
					...
					Text Tools
					<svg
						class="ml-1 h-4 w-4 fill-current transition-transform {dropOpen ? 'rotate-180' : ''}"
						viewBox="0 0 20 20"
					>
						<path
				...
				{#if dropOpen}
					<div
						class="dropdown-content animate-in slide-in-from-top-2 fade-in-0 menu z-50 mt-2 w-64 rounded-xl border border-base-200 bg-base-100 p-3 shadow-xl duration-200"
					>
						{#each menus as m}
							<a
	...
	{#if open}
		...
		>
			<div class="mx-auto max-w-sm space-y-2 p-4">
				...
				<div
					class="mt-4 px-3 py-2 text-xs font-semibold tracking-wider text-base-content/60 uppercase"
				>
					Text Tools
				</div>
				{#each menus as m}
					<a
						class="flex items-center rounded-lg px-3 py-3 transition-all duration-200 hover:bg-primary/10 {$page
							.url.pathname === m.href
